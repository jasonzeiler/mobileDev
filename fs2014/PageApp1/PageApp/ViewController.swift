...
class ViewController: UIViewController, UIPageViewControllerDelegate, UIPageViewControllerDataSource {
    ...
    // To loop through pages continuously A -> B -> C -> D -> A -> B -> C -> D, set loopPages to true
    ...
    func pageViewController(pageViewController:
        ...
        UIPageViewController, spineLocationForInterfaceOrientation orientation: UIInterfaceOrientation) ->
    ...
    func pageViewController(pageViewController: UIPageViewController, viewControllerAfterViewController viewController: UIViewController) -> UIViewController? { // Get the index position in the pages array for the current page to know what the next page should be. 
        let indexPosition = indexPositionForCurrentPage(pageViewController)
    ...
    if (nextIndexPosition > pages.count - 1) { if (loopPages) {
...
return pages[nextIndexPosition]
    ...
    func indexPositionForCurrentPage(pageViewController:UIPageViewController) -> Int {
            ...
            for (index, page) in enumerate(pages) { if (currentViewController == page) {
                return index
            }
